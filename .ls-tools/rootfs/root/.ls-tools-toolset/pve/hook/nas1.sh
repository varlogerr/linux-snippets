...
PVE_HOME=/root/.ls-tools-toolset/pve
...
# detect devmode
DEVMODE=false
cat /root/.ls-tools-conf/pve/devmode/${ID} &>/dev/null && {
...
if [[ " post-start " == *" ${PHASE} "* ]]; then
  ...
  LXC_PASS="$(cat "/root/.ls-tools-conf/pve/secrets/homelab/pass/${ID}.pass" 2>/dev/null)" \
  || LXC_PASS="$(cat "/root/.ls-tools-conf/pve/secrets/homelab/pass/master.pass" 2>/dev/null)" \
